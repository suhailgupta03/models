...
DROOT = '/home/suhail.gupta/models/_obd_' 

MODEL_NAME = DROOT + '/47246'
# Path to frozen detection graph. This is the actual model that is used for the object detection.
PATH_TO_CKPT = MODEL_NAME + '/frozen_inference_graph.pb'

PATH_TO_TEST_IMAGES_DIR = DROOT + '/test_images'

# List of the strings that is used to add correct label for each box.
PATH_TO_LABELS = os.path.join(DROOT + '/' + 'training', 'object-detection.pbtxt')
        ...
        #       line_thickness=8)
            ...
            for i in range(0, len(output_dict["detection_scores"])):
                ...
                if (output_dict["detection_scores"][i] > 0.5):
                    ...
                    box = {}
                    box["x_min"] = str(
                        (output_dict["detection_boxes"][i][1]) * width)
                    box["y_min"] = str(
                        (output_dict["detection_boxes"][i][0]) * height)
                    box["x_max"] = str(
                        (output_dict["detection_boxes"][i][3]) * width)
                    box["y_max"] = str(
                        (output_dict["detection_boxes"][i][2]) * height)
